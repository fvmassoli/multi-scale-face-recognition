...
class MTCNN(object):
    ...
    def __init__(self, caffe_model_path, gpu_flag):
        self.PNet = None
        self.RNet = None
        self.ONet = None
    ...
    def _load_model(self, caffe_model_path):
        ...
        # self.RNet = caffe.Net(rnet_proto, rnet_model, caffe.TEST)
        # self.ONet = caffe.Net(onet_proto, onet_model, caffe.TEST)
        self.PNet = cv2.dnn.readNetFromCaffe(pnet_proto, pnet_model)
        self.RNet = cv2.dnn.readNetFromCaffe(rnet_proto, rnet_model)
        self.ONet = cv2.dnn.readNetFromCaffe(onet_proto, onet_model)
    ...
    def _nms(self, boxes, threshold, type_):
        """nms
        :boxes: [:,0:5]
        :threshold: 0.5 like
        :type: 'Min' or others
    ...
    def _generateBoundingBox(self, map_, reg, scale, t):
        stride = 2
        cellsize = 12
        map_ = map_.T
        dx1 = reg[0, :, :].T
    ...
    def detect_face(self, img, bounding_box_detection_confidence=0.8):
        factor_count = 0
        ...
        for scale in scales:
            ...
            boxes = self._generateBoundingBox(out['prob1'][0, 1, :, :], out['conv4-2'][0], scale, self.threshold[0])
            if boxes.shape[0] != 0:
                # print(boxes[4:9])
        ...
        if numbox > 0:
            ...
            total_boxes = self._rerec(total_boxes)  # convert box to square
        ...
        if numbox > 0:
            ...
            if total_boxes.shape[0] > 0:
                ...
                if len(pick) > 0:
                    ...
                    total_boxes = self._rerec(total_boxes)
...
#        print('bb confidence', total_boxes[:, 4])
        total_boxes = total_boxes[total_boxes[:, 4] > bounding_box_detection_confidence, :4]
